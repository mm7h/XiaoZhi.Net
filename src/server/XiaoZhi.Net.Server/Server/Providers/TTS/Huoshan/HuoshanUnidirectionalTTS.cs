...
namespace XiaoZhi.Net.Server.Providers.TTS.Huoshan
{
    ...
    {
        private readonly ConcurrentQueue<OutSegment> _segmentsCache;
        ...
        {
            this._segmentsCache = new ConcurrentQueue<OutSegment>();
        }
        ...
        protected override (string, Emotion) GetSubtitle(Message message, bool isSentenceStart)
        {
            ...
            if (isSentenceStart)
            {
                if (this._segmentsCache.TryPeek(out OutSegment? seg) && seg is not null)
                {
            ...
            {
                if (this._segmentsCache.TryDequeue(out OutSegment? seg) && seg is not null)
                {
                    return (sentence, seg.Emotion);
                }
            }
