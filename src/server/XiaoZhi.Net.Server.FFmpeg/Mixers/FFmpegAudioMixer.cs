...
namespace XiaoZhi.Net.Server.FFmpeg.Mixers
{
    ...
    /// </summary>
    internal sealed class FFmpegAudioMixer : IAudioMixer
    {
        ...
        public FFmpegAudioMixer(ILogger<FFmpegAudioMixer>? logger = null)
        {
            _logger = logger ?? NullLogger<FFmpegAudioMixer>.Instance;
        ...
        public bool Initialize(int outputSampleRate, int outputChannels, int frameDuration, AudioMixerConfig? config = null)
        {
            lock (_syncLock)
            {
                try
                {
                    ...
                    {
                        { AudioType.SystemNotification, _config.SystemNotificationVolumeConfig.SoundSuppressionPercentage },
                        { AudioType.TTS, _config.TTSVolumeConfig.SoundSuppressionPercentage },
                        { AudioType.Music, _config.MusicVolumeConfig.SoundSuppressionPercentage },
        ...
        private float CalculateTargetVolume(AudioType audioType, int highestPriority, List<AudioType> activeTypes)
        {
            ...
            var suppressionLevel = _prioritySuppressionLevels?.GetValueOrDefault(audioType, 0.05f) ?? 0.05f;
            var higherPriorityCount = activeTypes.Count(t => (int)t > currentPriority);

            if (higherPriorityCount > 0)
            {
                suppressionLevel *= (float)Math.Pow(0.5, higherPriorityCount - 1);
            }

            return baseVolume * suppressionLevel;
        }
